...
def u_s_rho(s, rho, mat_id):
    ...
    ----------
    s : float
        Specific internal entropy (J K^-1 kg^-1).
    ...
    # u (in this and the next density slice of the 2D u array)
    idx_s_1_intp_s_1 = find_index_and_interp(log_s, A2_log_s[idx_rho])
    idx_s_1 = int(idx_s_1_intp_s_1[0])
    intp_s_1 = idx_s_1_intp_s_1[1]
    idx_s_2_intp_s_2 = find_index_and_interp(log_s, A2_log_s[idx_rho + 1])
    idx_s_2 = int(idx_s_2_intp_s_2[0])
    intp_s_2 = idx_s_2_intp_s_2[1]

    u_1 = A2_u[idx_rho, idx_s_1]
    u_2 = A2_u[idx_rho, idx_s_1 + 1]
    u_3 = A2_u[idx_rho + 1, idx_s_2]
    u_4 = A2_u[idx_rho + 1, idx_s_2 + 1]

    # If below the minimum s at this rho then just use the lowest table values
    if idx_rho >= 0 and (intp_s_1 < 0 or intp_s_2 < 0 or u_1 > u_2 or u_3 > u_4):
        intp_s_1 = 0
        intp_s_2 = 0
    ...
    # Unless already trying to extrapolate in which case return zero
    if num_non_pos > 0:
        if intp_rho < 0 or intp_s_1 < 0 or intp_s_2 < 0:
            return 0.0
        else:
            u_tiny = np.amin(np.abs(A2_u)) * 1e-3
            if u_1 <= 0:
    ...
    # u(rho, s)
    u = (1 - intp_rho) * ((1 - intp_s_1) * u_1 + intp_s_1 * u_2) + intp_rho * (
        (1 - intp_s_2) * u_3 + intp_s_2 * u_4
    )
